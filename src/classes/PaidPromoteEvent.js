...
class PaidPromoteEvent {
  constructor(eventData, issuer) {
    ...
    this.maxUploadedImagesByParticipant =
      Number(eventDetail.maxUploadedImagesByParticipant) > 0
        ? Number(eventDetail.maxUploadedImagesByParticipant)
        : 1;
  ...
  _validate() {
    const mandatoryDataToInstantiateValidationForm =
      process.env.LIST_MANDATORY_DATA_TO_INSTANTIATE_NEW_VALIDATION_FORM.split(
        ","
      );
    ...
    assert.notStrictEqual(
      ...
      "Invalid Date",
      new CustomError("Invalid date on startDate field.")
    );
    assert.notStrictEqual(
      String(this.endDate),
      "Invalid Date",
      new CustomError("Invalid date on endDate field.")
    );
    assert.notStrictEqual(
      this.maxUploadedImagesByParticipant < 1,
      true,
      new CustomError("Minimal image uploaded must be greater than 1")
    );
    ...
    for (let i = 0; i < mandatoryDataToInstantiateValidationForm.length; i++) {
      ...
      assert.notStrictEqual(
        this[keyName],
        undefined,
        `Please define ${keyName} so it's not null.`
      );
    }
  }
  ...
  async createEvent() {
    try {
      ...
      throw new CustomError(
        `System failed to create new Validation Form: ${this.eventName}.`,
        500
      );
    }
  }
  ...
  async _getOCRResult() {
    console.log(process.env.OCR_API_URL);
    try {
      ...
      console.log(
        "System failed to fetch ocr result from OCR api. This will make ocr result on DB storing null value. Full error message:",
        e
      );
    }
  }
  ...
  static getPaidPromoteData(body, file) {
    ...
    const fullFields =
      process.env.LIST_FULL_DATA_TO_INSTANTIATE_NEW_VALIDATION_FORM.split(",");
    ...
    configurationObject["baseImageNames"] =
      this.getBaseImagesPath(uploadedFiles);
  ...
  static async getMaxUploadedImagesFromEventID(eventID) {
    ...
    try {
      ...
      if (e instanceof mongoose.Error.CastError)
        throw new CustomError("Event ID not valid");
      throw new CustomError(
        "System failed to get necessary data to process handling agent.",
        500
      );
    }
  }
